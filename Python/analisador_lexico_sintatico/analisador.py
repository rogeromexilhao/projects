...
operadores=['-','+','*','/','<', '>']
relacao=['<>','>=','<=','=','<','>']
tipos_var=['integer','double','str','char','real']
tokensTOP = []
lista=[]
cont = 0
    ...
    for line in file:
        ...
        def discover_type(word):
            ...
            if 'procedure' in palavra_numero:
                ...
                print('Token: <dc_p>')
            if '(' in palavra_numero and ')' in palavra_numero:
                if 'write' in palavra_numero or 'read' in palavra_numero or 'procedure' in palavra_numero:
                    pass
                else:
                    aux2 = palavra_numero.split(' ')
                    if aux2[0].isidentifier():
...
def sintaticalAnaliser():
    ...
    print('')
    print('START SINTATICAL ANALISER -------------------------------------------------------------------------------')
    print('')
...
def verifica():
    global aux
    if aux[0] == '':
        if tokensTOP[1] == 'end' and tokensTOP[2] == 'begin':
           lista.pop(0)
           tokensTOP.pop(0)
        elif tokensTOP[0] == '<cmd>' and tokensTOP[1] == 'end':
            lista.pop(0)
            tokensTOP.pop(0)
            lista.pop(0)
            tokensTOP.pop(0)
